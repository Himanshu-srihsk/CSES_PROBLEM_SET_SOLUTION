...
public class PatternPositions {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<n;i++){
            ...
            if (terminal[i].firstPos == INF) {
                ans.append(-1).append("\n");
            } else {
                int start = terminal[i].firstPos - patternLen[i] + 2;
    ...
    private static void dfs(Node node) {
        ...
        for(Node v: children){
            dfs(v);
            node.firstPos = Math.min(node.firstPos,v.firstPos);
        }
    ...
    private static void processText(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            curr.firstPos = Math.min(curr.firstPos,i);
        }
    }
